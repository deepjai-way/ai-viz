 ...
 *
 * Generates multi-file structure similar to Claude Code skills format
 * with SKILL.md as the main entry and separate plugin files.
 */
class QoderAdapter extends BaseAdapter {
  constructor(config = {}) {
    super('Qoder', { toolId: 'qoder', ...config });
  }

  getOutputDir(projectRoot) {
    return path.join(projectRoot, 'skills', 'ai-viz');
  }

  async detect(projectRoot) {
    const skillsDir = path.join(projectRoot, 'skills');
    const qoderDir = path.join(projectRoot, '.qoder');
    return (await fs.pathExists(skillsDir)) || (await fs.pathExists(qoderDir));
  }

  /**
   * Install compiled content as Qoder skills
  ...
  /**
   * Build the main SKILL.md content (Qoder skills format)
   * @private
   */
